        ...
        <div class="col-lg-10">
            <div class="card shadow-lg">
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('admin.kriteria.store') }}" method="POST" id="kriteriaForm">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('kode_kriteria') is-invalid @enderror" 
                                           ...
                                           value="{{ old('kode_kriteria') }}" required placeholder="Contoh: K01">
                                    @error('kode_kriteria')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('prinsip') is-invalid @enderror" 
                                ...
                                <option value="Prinsip 1" {{ old('prinsip') == 'Prinsip 1' ? 'selected' : '' }}>Prinsip 1</option>
                                <option value="Prinsip 2" {{ old('prinsip') == 'Prinsip 2' ? 'selected' : '' }}>Prinsip 2</option>
                                <option value="Prinsip 3" {{ old('prinsip') == 'Prinsip 3' ? 'selected' : '' }}>Prinsip 3</option>
                                <option value="Prinsip 4" {{ old('prinsip') == 'Prinsip 4' ? 'selected' : '' }}>Prinsip 4</option>
                                <option value="Prinsip 5" {{ old('prinsip') == 'Prinsip 5' ? 'selected' : '' }}>Prinsip 5</option>
                            </select>
                            @error('prinsip')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div id="indikator-container">
                                ...
                                <div class="indikator-item">
                                    <div class="indikator-header">
                                        <h6 class="mb-0">Indikator 1</h6>
                                        <button type="button" class="btn-remove-indikator" onclick="removeIndikator(this)" style="display: none;">
                                            <i class="fas fa-times"></i>
                                        </button>
                                    </div>
                                    <textarea class="form-control indikator-field" name="indikator_fields[]" rows="3" required placeholder="Masukkan deskripsi indikator">{{ old('indikator_fields.0') }}</textarea>
                                </div>
                            </div>
                            ...
                            <input type="hidden" name="indikator" id="indikatorHidden" value="{{ old('indikator') }}">
                        </div>
                        
                        <div class="mb-3">
                            <label for="verifier" class="form-label">Verifier</label>
                            <textarea class="form-control @error('verifier') is-invalid @enderror" 
                                      id="verifier" name="verifier" rows="3" placeholder="Masukkan verifikasi yang diperlukan">{{ old('verifier') }}</textarea>
                            @error('verifier')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                                      id="deskripsi" name="deskripsi" rows="3" placeholder="Masukkan deskripsi tambahan (opsional)">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
<script>
    ...
    // Auto-format kode kriteria
    document.getElementById('kode_kriteria').addEventListener('input', function(e) {
        let value = e.target.value.toUpperCase();
        // Format: huruf diikuti angka (contoh: K01, P02, dll)
